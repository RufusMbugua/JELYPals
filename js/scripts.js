...
function date_time_create() {
	var dates = ['#date', '#inputDate', '#iodineDate1', '#iodineExpiration', '#reportingDate', '#fortifiedDate', '#checkupDate', '#dateC1', '#visitDate', '#inspection_date', '#supervision_date', '#inspector_date', '#inspectionsDate', '#signatureDate', '#supervisorDate', '#controlDate', '#premixDate', '#inspections_date', '#inspectionDate', '#roSignature', '#ho_signature_date', '#s_signature_date', '#externalIodB1_date_rep_signed'];
...
function mobi_scroller() {/*the time picker plugin*/
	...
	var t = 'default';
	var m = 'mixed';
	$('#startTime').scroller('destroy').scroller({
		preset : 'time',
		theme : t,
		mode : m
	});
	$('#endTime').scroller('destroy').scroller({
		preset : 'time',
		theme : t,
		mode : m
	});
	$('#productionTime').scroller('destroy').scroller({
		preset : 'time',
		theme : t,
		mode : m
	});
	$('#visitTime').scroller('destroy').scroller({
		preset : 'time',
		theme : t,
		mode : m
	});
